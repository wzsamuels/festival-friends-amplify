...
const NewRideModal = ({isOpen, setIsOpen, event} : NewRideModalProps) => {
  ...
  const onSubmit: SubmitHandler<RideInputs> = async data => {
    ...
    if (userProfile) {
      ...
      const newRide = await DataStore.save(new Ride({
      ...
      const updatedRide = await DataStore.save(Ride.copyOf(newRide, updated => {updated.driver = newRideUser;}))
  ...
  return (
    <Modal isOpen={isOpen} setIsOpen={setIsOpen} title="New Ride" className='max-w-xl'>
