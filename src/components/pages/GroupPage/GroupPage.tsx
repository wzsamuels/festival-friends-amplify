...
import {CollegeGroup, EventProfile, Festival, UserProfile} from "../../../models";
...
const GroupsPage = () => {
  ...
  useEffect(() => {
    ...
    if (authStatus === 'authenticated' && userProfile?.collegeGroup !== undefined && dataStoreCleared) {
  ...
  useEffect(() => {
    if(collegeGroup) {
      const collegeGroupSub = DataStore.observeQuery(Festival, (festival) => festival.groupID.eq(collegeGroup.id))
        .subscribe(({items}) => {
  ...
  if(authStatus !== 'authenticated' || (!collegeGroup && authStatus === 'authenticated')) {
    return (
      <>
        <Header/>
            ...
            <div className='p-4 m-4  text-center'>
              {
                ...
                <div>
                  <div className='mb-6 text-xl md:text-2xl lg:text-3xl'>Connect with your community</div>
                  <Link to='/account'>
    ...
    <>
      <Header/>
        <div className='p-4 mt-4'>
          <span className='text-xl md:text-2xl lg:text-3xl'>Welcome to the {collegeGroup?.name} community page!</span>
          <div className='grid gap-4 justify-items-center grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 items-center px-4'>
          {
            events?.map((event) => (
              <EventCard
                festival={event}
                key={event.id}
                attendingFriends={getAttendingFriends({eventAttendees: eventAttendees, friendProfiles: friendProfiles, eventId: event.id })}
              />
            ))
          }
          </div>
          <Button onClick={() => setIsCreateEventModalOpen(true)}>Create Event</Button>
        </div>
      { collegeGroup &&<CreateEventModal group={collegeGroup} isOpen={isCreateEventModalOpen} setIsOpen={setIsCreateEventModalOpen}/>}
    </>
  )
}
